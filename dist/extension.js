...
var JUMP_WINDOW_DAYS = 7;
function summarize(names, one, many) {
...
function collectAttention(input) {
  ...
  const items = [];
  const add = (kind, severity, view, text) => {
  ...
  for (const state2 of ["exhausted", "low"]) {
    const providers = input.accounts.filter((account) => account.state === state2).map((account) => account.provider);
    add("account", "warn", "accounts", summarize(
      providers,
      (name) => state2 === "exhausted" ? `${name}: Guthaben ersch\xF6pft` : `${name}: Guthaben wird knapp`,
      (count2) => state2 === "exhausted" ? `${count2} Konten ersch\xF6pft` : `${count2} Konten werden knapp`
    ));
  }
  const named = (status) => input.assessments.filter((item) => item.status === status).map((item) => item.agent.name);
  add("agent", "warn", "agents", summarize(
    [...named("deprecated"), ...named("unsuitable")],
    (name) => `Agent \u201E${name}" braucht ein anderes Modell`,
    (count2) => `${count2} Agenten brauchen ein anderes Modell`
  ));
  const cutoff = now - JUMP_WINDOW_DAYS * 864e5;
  const jumps = input.history.filter((change) => change.at >= cutoff && change.percent !== null && Math.abs(change.percent) >= input.jumpPercent);
  add("price", "info", "history", summarize(
    jumps.map((change) => change.modelId),
    (name) => `Deutliche Preis\xE4nderung bei ${name}`,
    (count2) => `${count2} deutliche Preis\xE4nderungen`
  ));
  add("agent", "info", "agents", summarize(
    named("alternative-available"),
    (name) => `F\xFCr \u201E${name}" gibt es eine g\xFCnstigere Alternative`,
    (count2) => `${count2} Agenten haben eine g\xFCnstigere Alternative`
  ));
  return items;
}
...
var MIGRATION_KEY = "priceWatch.migration.v1";
var MIGRATED_LAST_HASH_KEY = "priceWatch.lastHash.v1";
var LEGACY_LAST_HASH_KEY = "priceWatch.lastHash";
var LEGACY_LAST_AI_KEY = "priceWatch.lastAiAt";
var LEGACY_HISTORY_KEY = "priceWatch.history.v2";
var LEGACY_SNAPSHOT_KEY = "priceWatch.snapshots.v2";
var isRecord = (value) => typeof value === "object" && value !== null && !Array.isArray(value);
var isNonNegativeNumber = (value) => typeof value === "number" && Number.isFinite(value) && value >= 0;
function validPriceChange(value) {
...
function validOffers(value) {
  ...
  for (const raw of value) {
    ...
    let clean = true;
    for (const dimension of ["cacheRead", "cacheWrite", "request", "image", "webSearch"]) {
      const price = raw.pricing[dimension];
      if (price !== void 0 && !isNonNegativeNumber(price)) {
        clean = false;
        break;
      }
    }
    if (clean) offers.push(raw);
  }
  return offers;
}
function validProviderSnapshot(value) {
  ...
  const snapshot = { provider: value.provider, offers, checkedAt: value.checkedAt, stale: value.stale };
  if (isRecord(value.error)) snapshot.error = value.error;
  if (typeof value.warning === "string") snapshot.warning = value.warning;
  return snapshot;
}
async function migrateLegacyState(store, targets) {
  if (store.get(MIGRATION_KEY) !== void 0) return;
  const lastHash = store.get(LEGACY_LAST_HASH_KEY);
  if (typeof lastHash === "string" && lastHash.length > 0 && store.get(MIGRATED_LAST_HASH_KEY) === void 0) await store.update(MIGRATED_LAST_HASH_KEY, lastHash);
  await store.update(LEGACY_LAST_HASH_KEY, void 0);
  const lastAiAt = store.get(LEGACY_LAST_AI_KEY);
  if (typeof lastAiAt === "number" && Number.isFinite(lastAiAt) && lastAiAt > 0 && store.get(targets.aiLastRunKey) === void 0) await store.update(targets.aiLastRunKey, lastAiAt);
  await store.update(LEGACY_LAST_AI_KEY, void 0);
  const legacyHistory = store.get(LEGACY_HISTORY_KEY);
  if (Array.isArray(legacyHistory) && store.get(targets.historyKey) === void 0) {
    const migrated = legacyHistory.filter(validPriceChange);
    if (migrated.length > 0) await store.update(targets.historyKey, migrated);
  }
  await store.update(LEGACY_HISTORY_KEY, void 0);
  const legacySnapshots = store.get(LEGACY_SNAPSHOT_KEY);
  if (Array.isArray(legacySnapshots) && store.get(targets.snapshotKey) === void 0) {
    const migrated = [];
    for (const raw of legacySnapshots) {
      const item = validProviderSnapshot(raw);
      if (item) migrated.push(item);
    }
    if (migrated.length > 0) await store.update(targets.snapshotKey, migrated);
  }
  await store.update(LEGACY_SNAPSHOT_KEY, void 0);
...
var dimensionFor = (name) => {
  switch (name) {
    case "gpqa_diamond":
      return "intelligence";
    case "arena_codecategories":
      return "coding";
...
function aggregateBenchmarkScores(details) {
  ...
  const scores = {};
  for (const [dimension, { sum, count: count2 }] of sums) {
    scores[dimension] = Math.round(sum / count2 * 10) / 10;
  }
  return scores;
}
...
function purposeBadge(purpose) {
  return `<span class="badge purpose purpose-${purpose}"><b aria-hidden="true">${purposeIcon[purpose]}</b>${labels[purpose]}</span>`;
}
function providerBadge(provider) {
  return `<span class="badge provider provider-${provider}"><i aria-hidden="true"></i>${esc(provider === "openrouter" ? "OpenRouter" : provider === "opencode-zen" ? "Zen" : "Go")}</span>`;
}
...
var statusLabel = { suitable: "Passend", "alternative-available": "Alternative", unsuitable: "Unpassend", deprecated: "Veraltet", local: "Lokal", unknown: "Nicht bewertbar" };
...
var dimensionLabel = { input: "Input", output: "Output", cacheRead: "Cache gelesen", cacheWrite: "Cache geschrieben", request: "je Anfrage" };
function percentCell(change) {
...
function historyRows(history) {
  if (!history.length) return `<p class="empty">Noch keine Preis\xE4nderungen aufgezeichnet</p>`;
...
function renderAttention(items = []) {
  ...
  return items.map((item) => `<button class="attention-item ${item.severity}" data-view="${item.view}" aria-label="${esc(item.text)} \u2013 Ansicht \xF6ffnen">${esc(item.text)}</button>`).join("");
}
function renderHistoryCard(history) {
  return `<div class="card-head"><h2>Preisverlauf</h2><button data-view="history" aria-label="Alle ${history.length} Preis\xE4nderungen im Verlauf \xF6ffnen">Alle ${history.length}</button></div>${history.length ? `<div class="change-rows change-preview">${historyRows(history.slice(0, 3))}</div>` : `<p class="empty">Noch keine Preis\xE4nderungen</p>`}`;
}
function renderRanks(offers) {
  return Object.entries(labels).map(([purpose, label], index) => {
    const column = (mode) => {
      ...
      return `<section class="rank-column price-${mode}"><h4><i aria-hidden="true"></i>${title}</h4>${ranked.length ? `<ol>${ranked.map((item) => `<li><strong>${esc(item.offer.name)}</strong><small>Score ${item.score} \xB7 ${money(item.offer.pricing.input)} / ${money(item.offer.pricing.output)}</small></li>`).join("")}</ol>` : `<p class="empty">Keine belastbar bewerteten Modelle</p>`}</section>`;
    };
...
var overviewAccountsInner = ({ state: state2 }) => `<div class="card-head"><h2>Konten &amp; Limits</h2><button data-view="accounts" aria-label="Konten und Limits Details \xF6ffnen">Details</button></div>${state2.accounts.length ? state2.accounts.map(renderAccountSummary).join("") : `<p class="empty">Noch kein Konto verbunden</p>`}`;
...
function fragments(state2) {
  ...
  return {
    ...
    attention: renderAttention(state2.attention),
...
function panelHtml(state2) {
  ...
  return `<!doctype html><html lang="de"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width"><meta http-equiv="content-security-policy" content="default-src 'none'; img-src 'none'; style-src 'unsafe-inline'; script-src 'nonce-${nonce}'"><style>${CSS}${BENCHMARK_CSS}</style></head><body><header class="topbar"><button class="brand" data-view="overview" aria-label="Preis-Watch \xDCbersicht">Preis-Watch</button><nav role="navigation" aria-label="Ansichten"><button data-view="overview" class="active" aria-current="page" aria-label="\xDCbersicht">\xDCbersicht</button><button data-view="models" aria-label="Modelle">Modelle</button><button data-view="agents" aria-label="Agenten">Agenten</button><button data-view="history" aria-label="Verlauf">Verlauf</button><button data-view="accounts" aria-label="Konten und Limits">Konten &amp; Limits</button></nav><span class="live" aria-label="Daten aktuell"><i aria-hidden="true"></i>aktuell</span></header><main role="main">
...
function recomputeAttention() {
  ...
  state.attention = collectAttention({
    ...
    jumpPercent: Math.max(1, vscode.workspace.getConfiguration("priceWatch").get("priceJumpPercent", 20))
  });
}
