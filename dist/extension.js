...
var FIVE_MIN = 5 * 6e4;
var DAY = 24 * 36e5;
function liveLabel(state2) {
  if (state2.refreshError) return { cls: "live-error", label: "Aktualisierung fehlgeschlagen", text: "Fehler", title: state2.refreshError };
  const updated = state2.updatedAt;
...
var liveInner = ({ state: state2 }) => {
  const { cls, label, text, title } = liveLabel(state2);
