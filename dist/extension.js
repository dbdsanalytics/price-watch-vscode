...
function quotaLine(offer) {
  ...
  if (!quota) return "";
  const parts = [quota.requestsPerMonth !== void 0 ? `${count(quota.requestsPerMonth)} Anfragen/Monat` : "", quota.includedUsdPerMonth !== void 0 ? `${money(quota.includedUsdPerMonth)} enthalten` : ""].filter(Boolean);
  return parts.length ? `<small class="quota">${esc(parts.join(" \xB7 "))}</small>` : "";
}
...
function panelHtml(state2) {
  ...
  const modelRows = offers.slice().sort((a, b) => a.name.localeCompare(b.name)).map((offer) => `<tr data-model="${esc(`${offer.name} ${offer.provider} ${offer.capabilities.purposes.join(" ")}`.toLowerCase())}" data-provider="${offer.provider}" data-price="${priceClass(offer)}"><td><strong>${esc(offer.name)}</strong><small>${esc(offer.id)}</small>${quotaLine(offer)}</td><td>${providerBadge(offer.provider)}</td><td><span class="price price-${priceClass(offer)}">${offer.pricing.unknown ? "Preis unbekannt" : money(offer.pricing.input)}</span></td><td><span class="price price-${priceClass(offer)}">${offer.pricing.unknown ? "Preis unbekannt" : money(offer.pricing.output)}</span></td><td><div class="capabilities">${offer.capabilities.purposes.map(purposeBadge).join("")}</div></td><td>${benchmarkCell(offer)}</td></tr>`).join("");
