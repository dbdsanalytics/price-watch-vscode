...
const metricsInner = ({ state, offers, free }: PreparedView) => `<span><strong>${offers.length}</strong>Modelle</span><span><strong>${free}</strong>kostenlos</span><button class="metric-link" data-view="history"><strong>${state.history.length}</strong>Änderungen</button><span><strong>${state.agents.length}</strong>Agenten</span>`
